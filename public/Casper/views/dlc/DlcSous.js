...
const DlcSous = Vue.component('DlcSous', {
	data: function () {
	    return {
	    	options: {
		    	json:false,
...
<div id="listing">
  ...
  <v-data-table v-if="security.readable" :headers="computedHeaders" :items="dataItems" @current-items="eventFilterItems" :search="search" locale="fr" @click:row="showItem"  class="elevation-1" :footer-props="datatable.footerProps" :items-per-page="options.defaultSize">
    <template v-slot:top>
      <v-toolbar flat >
...
` ,  
    ...
    methods: {
	  initialize () {
            	axios.get('/Casper/v1/dlc/subscriptions', {
            		  headers: {
